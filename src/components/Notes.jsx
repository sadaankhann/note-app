...
const Notes = () => {
  ...
  return (
    ...
    <ThemeContext.Provider value={{Block, setBlock, elementsColor, setElementsColor}}>

      <div className={`block-1 grid grid-cols-2 w-full divide-x-3 divide-dashed min-h-screen transition-colors duration-300 delay-[600ms] ease-in  max-md:divide-x-0 max-md:grid-cols-1  ${(Block == "black" ? 'bg-black' : 'bg-white')} ${elementsColor == 'white' ? 'text-white' : 'text-black'}`}>
        <form onSubmit={(e) => {
          preventingDefault(e);
        }} className=''>
